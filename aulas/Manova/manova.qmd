...
modelo = manova(cbind(x1, x2, x3, x4) ~ tipo, data = dados)
...
```{r}
dados %>% emmeans_test(x1 ~ tipo, p.adjust.method = "bonferroni", detailed = TRUE)

dados %>% emmeans_test(x2 ~ tipo, p.adjust.method = "bonferroni", detailed = TRUE)
```

## Intervalos simultâneos de confiança para efeitos de tratamento

```{r}
dados %>% emmeans_test(x3 ~ tipo, p.adjust.method = "bonferroni", detailed = TRUE)

dados %>% emmeans_test(x4 ~ tipo, p.adjust.method = "bonferroni", detailed = TRUE)
```
## Intervalos simultâneos de confiança para efeitos de tratamento
...
```{r}
pwc <- dados %>%
  gather(key = "variables", value = "value", x1, x2,x3,x4) %>%
  group_by(variables) %>%
